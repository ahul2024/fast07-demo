...
app.get('/api/game/history',auth,async(req,res)=>{
  try{
    ...
    const r=await q(`SELECT r.period,
      ...
      WHERE r.period <= $2 ORDER BY r.period DESC LIMIT $3 OFFSET $4`,[req.user.id,String(current),limit+1,offset]);
    const rows=r.rows.slice(0,limit);
    res.json({currentPeriod:String(current),page,limit,hasNext:r.rows.length>limit,history:rows});
