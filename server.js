...
app.post('/api/auth/signup',async(req,res)=>{try{const {name,mobile,email='',password,referralCode=''}=req.body;const normalizedMobile=String(mobile||'').trim();const normalizedEmail=String(email||'').trim().toLowerCase()||('u'+normalizedMobile.replace(/\D/g,'')+'@fast07.local');if(!name||!normalizedMobile||!password)return res.status(400).json({error:'Missing required fields'});const ex=await q('SELECT id FROM users WHERE email=$1 OR mobile=$2',[normalizedEmail,normalizedMobile]);if(ex.rows[0])return res.status(409).json({error:'Mobile number already registered'});let ref=null;if(referralCode)ref=(await q('SELECT id FROM users WHERE referral_code=$1',[referralCode.toUpperCase()])).rows[0];const hash=await bcrypt.hash(password,12);const code='REF-'+crypto.randomBytes(4).toString('hex').toUpperCase();const r=await q('INSERT INTO users(name,mobile,email,password_hash,referral_code,referred_by,coins) VALUES($1,$2,$3,$4,$5,$6,$7) RETURNING id,name,mobile,email,status,coins,referral_code,created_at,last_seen,last_page',[name,normalizedMobile,normalizedEmail,hash,code,ref?.id||null,ref?1200:1000]);const u=r.rows[0];if(ref){await q('UPDATE users SET coins=coins+100 WHERE id=$1',[ref.id]);await q("INSERT INTO transactions(user_id,type,amount,status,meta) VALUES($1,'REFERRAL',100,'APPROVED',$2)",[ref.id,JSON.stringify({newUserId:u.id})])}await q("INSERT INTO activities(user_id,action,details,coins) VALUES($1,'REGISTER','New demo account created',$2)",[u.id,u.coins]);res.json({token:token(u),user:u})}catch(e){console.error(e);res.status(500).json({error:'Signup failed'})}});
