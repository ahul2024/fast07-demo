...
async function signup(){
  const name=el('sName').value.trim(),mobile=el('sMobile').value.trim(),referralCode=el('sRef').value.trim().toUpperCase();
  if(!name||!mobile)return el('msg').textContent='Please enter name and mobile number.';
  const email=mobile.replace(/\D/g,'')+'@fast07.local';
  const pass=generateSignupPassword();
  ...
  try{
    ...
    el('lId').value=mobile; el('lPass').value=pass;
    el('msg').textContent='✓ Registration successful. Your generated password is filled in below. Save it before leaving this page.';
...
function updateHistoryPagination(hasNext){
  const prev=el('historyPrev'),next=el('historyNext'),info=el('historyPageInfo');
  if(prev)prev.disabled=historyPage<=1;
  if(next)next.disabled=!hasNext;
  if(info)info.textContent='Page '+historyPage;
}
async function changeHistoryPage(delta){
  const nextPage=Math.max(1,historyPage+delta);
  if(nextPage===historyPage)return;
  historyPage=nextPage;
  await renderAdvancedHistory();
}
async function renderAdvancedHistory(){
  const e=el('history'); if(!e)return;
  try{
    const d=await api('/api/game/history?page='+historyPage+'&limit='+HISTORY_PAGE_SIZE), rows=d.history||[];
    ...
    updateHistoryPagination(!!d.hasNext);
    if(el('recent') && historyPage===1)el('recent').innerHTML=rows.slice(0,8).map(x=>`<div class="history-item"><span>#${esc(x.period)}</span><span>${esc(x.number??'--')}</span><span>${esc(x.colour??'--')}</span><b>${esc(x.bet_outcome|| (String(x.period)===String(d.currentPeriod)?'LIVE':'NO BET'))}</b></div>`).join('')||'<small>No rounds yet.</small>';
  }catch(e){
    const h=safeJSON(key(H),[]),e2=el('history');
    const start=(historyPage-1)*HISTORY_PAGE_SIZE, rows=h.slice(start,start+HISTORY_PAGE_SIZE);
    if(e2)e2.innerHTML=rows.length?rows.map(x=>`<div class="history-item"><span>#${esc(x.period||x.r)}</span><span>${esc(x.betChoice||'No Bet')}</span><span>${esc(x.size||'--')}</span><span>${esc(x.colour||'--')}</span><span>${esc(x.number??'--')}</span><span>🪙 ${esc(x.bet||0)}</span><b>${esc(x.status||'--')}</b></div>`).join(''):'<small>No rounds yet.</small>';
    updateHistoryPagination(start+HISTORY_PAGE_SIZE<h.length);
  }
}
...
setInterval(async()=>{const u=ensureCurrentUser();if(u){if(u.status==='BLOCKED'){logout();return}updateUser({lastSeen:Date.now()});refresh();if(!el('wallet')?.classList.contains('hidden'))await renderTransactions();}},3000);
...
// Refresh-persistent, non-wagering practice countdown.
let practiceTimerInterval = null;
const PRACTICE_TIMER_KEY = 'fast07_practice_timer_end';
const PRACTICE_TIMER_DURATION_KEY = 'fast07_practice_timer_duration';
...
function renderPracticeTimer(){
  ...
  if(!display) return;
  const end=Number(localStorage.getItem(PRACTICE_TIMER_KEY)||0);
  const duration=Number(localStorage.getItem(PRACTICE_TIMER_DURATION_KEY)||60);
  if(!end){
    display.textContent=formatPracticeTime(duration);
    if(status) status.textContent='Ready';
    return;
  }
  const remaining=Math.max(0,(end-Date.now())/1000);
  display.textContent=formatPracticeTime(remaining);
  if(remaining<=0){
    localStorage.removeItem(PRACTICE_TIMER_KEY);
    if(status) status.textContent='Finished';
  } else if(status) status.textContent=`${duration}-second practice timer running`;
}

function setPracticeTimer(seconds){
  localStorage.setItem(PRACTICE_TIMER_DURATION_KEY,String(seconds));
  localStorage.setItem(PRACTICE_TIMER_KEY,String(Date.now()+seconds*1000));
  renderPracticeTimer();
  startPracticeTimerLoop();
}

function startPracticeTimerLoop(){
  if(practiceTimerInterval) clearInterval(practiceTimerInterval);
  practiceTimerInterval=setInterval(()=>{
    renderPracticeTimer();
    if(!localStorage.getItem(PRACTICE_TIMER_KEY)) {
...
window.addEventListener('load',()=>{ renderPracticeTimer(); if(localStorage.getItem(PRACTICE_TIMER_KEY)) startPracticeTimerLoop(); });
