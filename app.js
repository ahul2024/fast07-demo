...
async function signup(){
  const name=el('sName').value.trim(),mobile=el('sMobile').value.trim(),pass=el('sPass').value,referralCode=el('sRef').value.trim().toUpperCase();
  if(!name||!mobile||!pass)return el('msg').textContent='Please fill all fields.';
  el('msg').textContent='Creating account...';
  try{
    await api('/api/auth/signup',{method:'POST',body:JSON.stringify({name,mobile,password:pass,referralCode})});
    ...
    el('lId').value=mobile; el('lPass').value='';
